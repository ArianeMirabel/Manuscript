...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Le script retourne la moyenne et l'intervalle de confiance à 95% du profil}
\CommentTok{# de diversité d'un inventaire.  L'inventaire est une matrice Plot (individu}
\CommentTok{# * données botaniques), les données manquantes sont indiquées par 'Indet.'}

\KeywordTok{head}\NormalTok{(Plot, }\DecValTok{5}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##      Vernacular        Family     Genus    Species                name
## 626  busi mango    Clusiaceae  Tovomita     Indet.     Tovomita Indet.
## 3   maho cochon     Malvaceae Sterculia   pruriens  Sterculia pruriens
## 8    maho rouge Lecythidaceae  Lecythis persistens Lecythis persistens
## 4          wapa      Fabaceae    Eperua    falcata      Eperua falcata
## 815        weko      Fabaceae      Inga     Indet.         Inga Indet.
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# La première étape donne une matrice d'association vernaculaire/botanique.}
\CommentTok{# La matrice alpha liste les fréquences d'association de référence.}
...
\CommentTok{# A partir d'un vecteur d'association, un processus Dirichlet/Multinomial}
\CommentTok{# permet de tirer au hasard un nom botanique}
...
\CommentTok{# Pour tenir compte de toute les informations phylogénétiques disponibles,}
\CommentTok{# on liste les associations possibles nom vernaculaire / Famille / genre /}
\CommentTok{# espèce}
...
\CommentTok{# On peut alors attribuer un nom botanique à tout nom vernaculaire:}
...
\NormalTok{        mismatch <-}\StringTok{ }\NormalTok{bota[}\KeywordTok{intersect}\NormalTok{(}\KeywordTok{which}\NormalTok{(bota[, }\StringTok{"Vernacular"}\NormalTok{] }\OperatorTok{==}\StringTok{ }\NormalTok{name[}\StringTok{"Vernacular"}\NormalTok{]), }
            \KeywordTok{which}\NormalTok{(}\OperatorTok{!}\NormalTok{bota[, }\StringTok{"Genus"}\NormalTok{] }\OperatorTok{%in%}\StringTok{ }\NormalTok{name[}\StringTok{"Genus"}\NormalTok{])), }\StringTok{"name"}\NormalTok{]}
...
\NormalTok{    \}}
    ...
    \CommentTok{# Restrict the sampling pool to account for phylogenetic information}
...
\NormalTok{        mismatch <-}\StringTok{ }\NormalTok{bota[}\KeywordTok{intersect}\NormalTok{(}\KeywordTok{which}\NormalTok{(bota[, }\StringTok{"Vernacular"}\NormalTok{] }\OperatorTok{==}\StringTok{ }\NormalTok{name[}\StringTok{"Vernacular"}\NormalTok{]), }
            \KeywordTok{which}\NormalTok{(}\OperatorTok{!}\NormalTok{bota[, }\StringTok{"Family"}\NormalTok{] }\OperatorTok{%in%}\StringTok{ }\NormalTok{name[}\StringTok{"Family"}\NormalTok{])), }\StringTok{"name"}\NormalTok{]}
\NormalTok{        alpha_adjust[}\KeywordTok{as.character}\NormalTok{(mismatch)] <-}\StringTok{ }\DecValTok{0}
\NormalTok{        match <-}\StringTok{ }\KeywordTok{names}\NormalTok{(alpha_adjust[}\KeywordTok{which}\NormalTok{(alpha_adjust }\OperatorTok{!=}\StringTok{ }\DecValTok{0}\NormalTok{)])}
\NormalTok{        alpha_adjust[}\KeywordTok{which}\NormalTok{(alpha_adjust }\OperatorTok{==}\StringTok{ }\DecValTok{0}\NormalTok{)] <-}\StringTok{ }\NormalTok{eps}\OperatorTok{/}\KeywordTok{length}\NormalTok{(}\KeywordTok{which}\NormalTok{(alpha_adjust }\OperatorTok{==}\StringTok{ }
\StringTok{            }\DecValTok{0}\NormalTok{))}
...
\CommentTok{# On peut alors générer un inventaire théorique complet à partir de}
\CommentTok{# l'inventaire réel}
...
\CommentTok{# L'algorithme final retourne la médiane et l'intervalle de confiance de}
\CommentTok{# l'estimateur de diversité pour tout ordre Q, après Nrep simulations}

\NormalTok{Est_Entropy <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(Plot, Q) \{}
...
\NormalTok{        entrop <-}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(}\KeywordTok{Replacement}\NormalTok{(Plot, }\DataTypeTok{Alpha =}\NormalTok{ alpha, }\DataTypeTok{Bota =}\NormalTok{ bota))}
...
\NormalTok{    Entrop <-}\StringTok{ }\KeywordTok{lapply}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\FloatTok{0.025}\NormalTok{, }\FloatTok{0.5}\NormalTok{, }\FloatTok{0.975}\NormalTok{), }\ControlFlowTok{function}\NormalTok{(p) \{}
        \KeywordTok{return}\NormalTok{(}\KeywordTok{quantile}\NormalTok{(x, }\DataTypeTok{probs =}\NormalTok{ p, }\DataTypeTok{na.rm =}\NormalTok{ T))}
\NormalTok{    \})}
    ...
    \KeywordTok{return}\NormalTok{(}\KeywordTok{unlist}\NormalTok{(Entrop))}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=0.8\linewidth]{Manuscript_files/figure-latex/RedFunRest-1} 
