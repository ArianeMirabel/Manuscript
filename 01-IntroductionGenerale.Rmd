...
Pour des communautés végétales la similarité entre espèces peut être phylogénétique, lorsque l'on considère leur distance dans un arbre phylogénétique, ou fonctionnelle, lorsque l'on considère leurs différences morphologiques ou physiologiques \@ref(fig:RichEquSim).
...
En complément de la diversité la structure fonctionnelle des communautés inclut leur composition fonctionnelle, mesurable par les valeurs moyennes de traits à l'échelle de la communauté (*Community Weighted Means, CWM* en anglais) pondérées par l'abondance des espèces.
L'abondance des caractéristiques fonctionnelles détermine à la fois le fonctionnement des communautés et leur résilience.
D'après la théorie du "ratio de biomasse" [@Grime1998], le rôle d'un individu dans l'écosystème dépend de la fraction de biomasse qu'il représente et donc le fonctionnement des communautés repose sur les caractéristiques fonctionnelles des espèces dominantes et les espèces rares n'ont que peu d'influence.
Par ailleurs la répartition d'abondance des traits fonctionnels dans l'écosystème amène à la notion de redondance fonctionnelle qui quantifie le nombre d'espèces partageant les mêmes valeurs de traits.
La redondance entre espèces, souvent élevée en forêt tropicale, permet aux communautés de perdre des espèces sans nécessairement voir disparaître leur rôle dans l'écosystème: la redondance détermine en partie la résilience des communautés et atténue l'impact des perturbations.
L'organisation de la redondance dans l'espace des traits d'une communauté désigne ainsi les assemblages les plus stables et renseigne sur le communautés qui s'installeraient le plus probablement après de nouvelles perturbations.
La redondance fonctionnelle d'une communauté peut se mesurer dans l'espace fonctionnel à partir des densités de probabilité de traits (*Traits Density Probability, TDP* en anglais) pour chacune des espèce s[@2016Carmona].
Les densités des espèces d'une communauté sont pondérées par leur abondance et additionnées pour avoir une valeur de redondance sur l'ensemble de l'espace fonctionnel ou sur un espace restreint, comme nous le ferons par la suite quand on s'intéressera à la redondance fonctionnelle dans l'espace de la communauté de départ.

```{r RichEquSim, out.width='60%', fig.cap="La redondance fonctionnelle est la somme des chevauchement entre espèces dans l'espace fonctionnel. Les individus de la base de données fonctionnelle sont représentés dans un espace à 2 dimensions grâce à une analyse en composantes principales (ACP). Une estimation par noyau estime ensuite la densité de probabilité des traits (TDP) de chaque espèce. La somme de ces densités pondérées par l'abondance des espèces donne enfin la redondance fonctionnelle de la communauté, interprétable comme le nombre d'espèces qui peuvent disparaître sans diminuer l'espace fonctionnel de la communauté."}
